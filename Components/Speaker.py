...
def detect_faces_and_speakers(input_video_path, output_video_path):
    ...
    while cap.isOpened():
        ...
        active_box = None  # Initialize candidate bounding box
        MaxDif = 0
        ...
        for i in range(detections.shape[2]):
            ...
            if confidence > 0.3:  # Confidence threshold
                ...
                # Assuming lips are approximately at the bottom third of the face
                lip_distance = abs((y + 2 * face_height // 3) - (y1))
                # Update MaxDif using assignment operator
                if lip_distance > MaxDif:
                    MaxDif = lip_distance
                    active_box = [x, y, x1, y1]

        # Combine visual and audio cues if we have a candidate
        if active_box is not None:
            if voice_activity_detection(audio_frame, sample_rate):  # active speaker if audio is speaking
                cv2.putText(frame, "Active Speaker", (active_box[0], active_box[1] - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            Frames.append(active_box)
        else:
            # No candidate found; optionally append a default value or handle accordingly
